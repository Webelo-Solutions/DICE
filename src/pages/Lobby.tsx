  ...
  return (
    <div className="min-h-screen bg-terminal-bg font-mono flex flex-col items-center p-6">
      <div className="w-full max-w-lg space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-[10px] text-terminal-dim tracking-widest uppercase">
              Lobby{isDepartmental && ' · Departmental'}
            </div>
        ...
        {/* Roster (live via WebSocket). Departmental sessions group by
            department and lead with role staffing, because an unstaffed role
            is skipped in play and the lobby is the last chance to fix it. */}
        {isDepartmental ? <DepartmentRoster /> : (
        ...
        <div className="rounded border border-terminal-border bg-terminal-surface/40 px-4 py-3 text-[11px] text-terminal-dim leading-relaxed">
          {isFacilitator
            ? isDepartmental
              ? 'You are the facilitator. Check that every role you need is staffed — unstaffed roles are skipped entirely once play starts — then pick a scenario and run the session.'
              : 'You are the facilitator. Pick a scenario and run the session as usual — every participant sees the incident unfold live.'
            : membership.role === 'dept_lead'
              ? 'You lead your department. Waiting for the facilitator to start the session — you will join the shared incident automatically.'
