...
import { GAME_ROLES, ROOM_CODE_LENGTH, countStaffing } from '../types/room'
...
export function JoinGame() {
  ...
  // The lookup passes the signed-in token: the room endpoint withholds the
  // roster and departments from anonymous callers, and joining requires an
  // account anyway. Without a token the peek still names the room, so the
  // staffing counts fall back to empty rather than assuming a roster.
  useEffect(() => {
    const trimmed = code.trim().toUpperCase()
    if (trimmed.length < 4) { setPeek(null); return }
    let cancelled = false
    roomApi.getLobby(trimmed, useUserStore.getState().token ?? undefined)
      .then(({ room, participants, departments }) => {
        if (cancelled) return
        const roster = participants ?? []
        setPeek({
          mode: room.mode, name: room.name, departments: departments ?? [], participants: roster,
          staffing: countStaffing(roster),
        })
      })
